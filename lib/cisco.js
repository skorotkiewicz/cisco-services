...
export const services = [
  { name: "Weather Forecast", url: `${urlServer}/Cisco/Weather` },
  { name: "Phone Book", url: `${urlServer}/Cisco/PhoneBook` },
  // { name: "Lotto Results", url: `${urlServer}/Cisco/Lotto` },
  { name: "Ping", url: `${urlServer}/Cisco/Ping` },
];
...
router.get("/Cisco/Weather", async (_req, res) => {
  try {
    ...
    sendErrorResponse(res, `Error fetching weather data: ${error}`);
  }
});
...
Conditions: ${forecast.condition}
    ...
    sendErrorResponse(res, `Error generating weather details: ${error}`);
  }
});
...
Nastepne losowanie: ${lottoData.nextDrawDate}
    ...
    sendErrorResponse(res, `Error generating lotto response: ${error}`);
  }
});
  ...
  try {
    ...
    const xml = `
      <CiscoIPPhoneText>
        ...
        <Text>Ping: ${pingResult.time} ms</Text>
      </CiscoIPPhoneText>`;
